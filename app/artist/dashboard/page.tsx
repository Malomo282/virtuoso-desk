      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="p-8 max-w-3xl mx-auto w-full">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground/80 text-sm">Virtuoso Entertainment Ltd</p>
          </div>

          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            <div className="bg-card border border-border rounded-xl p-5">
              <div className="text-muted-foreground/80 text-xs uppercase tracking-widest mb-2">Upcoming gigs</div>
              <div className="text-3xl font-bold text-white">{upcoming.length}</div>
            </div>
            <div className="bg-card border border-border rounded-xl p-5">
              <div className="text-muted-foreground/80 text-xs uppercase tracking-widest mb-2">Next gig</div>
              <div className="text-lg font-semibold text-primary">
                {nextGig ? new Date(nextGig.starts_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) : 'None booked'}
              </div>
            </div>
            <div className="bg-card border border-border rounded-xl p-5">
              <div className="text-muted-foreground/80 text-xs uppercase tracking-widest mb-2">Total earnings</div>
              <div className="text-3xl font-bold text-success">GBP {totalEarnings.toLocaleString()}</div>
            </div>
          </div>

          <div className="bg-card border border-border rounded-xl p-5 mb-8">
            <div className="text-muted-foreground/80 text-xs uppercase tracking-widest mb-4">Upcoming bookings</div>
            {upcoming.length === 0 ? (
              <div className="text-muted-foreground/60 text-sm">No upcoming gigs booked yet.</div>
            ) : (
          ...
          >
            ...
            <div className="text-muted-foreground/80 text-sm">Update your bio, genres, photo, and minimum fee</div>
          </div>
        </div>
      </div>
    </div>
