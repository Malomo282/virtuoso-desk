        ...
        <div className="p-8">
          {bookings.length === 0 && (
            <div className="text-center py-16">
              ...
              <div className="text-[#4E5A6A] text-sm">No bookings need urgent confirmation</div>
            </div>
          )}
          <div className="flex flex-col gap-3">
            {bookings.map(b => {
              ...
              return (
                <div key={b.id} className="bg-[#151A22] border border-[#263044] border-l-4 border-l-red-500 rounded-xl p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="text-white font-semibold">{b.venues?.name}</div>
                      {b.event_name && <div className="text-[#6A7A8A] text-xs mt-0.5">{b.event_name}</div>}
                      <div className="flex gap-3 text-xs text-[#6A7A8A] mt-2 flex-wrap">
                        ...
                        {b.artists?.stage_name && <span>{b.artists.stage_name}</span>}
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-2">
                      <span className="text-[#C8A24A] font-bold">GBP {(b.fee_venue || 0).toLocaleString()}</span>
                  ...
                  <div className="mt-3 pt-3 border-t border-[#263044] flex gap-2">
                    <button onClick={() => confirmBooking(b.id)} className="bg-[#C8A24A] text-[#0B0D10] text-xs font-bold px-4 py-2 rounded-lg hover:bg-[#D6B25E] transition-colors">Confirm now</button>
                    <button onClick={() => router.push('/agency/bookings')} className="bg-[#1C2330] border border-[#263044] text-[#6A7A8A] text-xs px-4 py-2 rounded-lg hover:text-white transition-colors">View booking</button>
                  </div>
                </div>
              )
            })}
